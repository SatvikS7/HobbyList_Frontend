...
const DiscoveryPage = () => {
  const [activeTab, setActiveTab] = useState<'people' | 'hobbies'| 'requests'>('people');
  const [users, setUsers] = useState<UserSummaryDto[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [requests, setRequests] = useState<UserSummaryDto[]>([]);
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      setError(null);
      const fetchedRequests = await followService.getRequests();
            ...
            {activeTab === 'people' ? (
              <>
                {loading ? (
                  ...
                  <div className="space-y-4">
                    {users.map(user => (
                      <UserDiscoveryItem key={user.id} user={user} mode="follow" onRefresh={refreshData} />
                    ))}
                  </div>
                )}
              </>
              ...
              <>
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    <p className="text-gray-400">No active follow requests</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {requests.map(user => (
                      <UserDiscoveryItem key={user.id} user={user} mode="request" onRefresh={refreshData} />
                    ))}
                  </div>
                )}
              </>
